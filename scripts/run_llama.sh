#!/usr/bin/env bash
# =============================================================================
# run_llama.sh — Interactive experiment reproduction for Meta-Llama-3.1-8B-Instruct
#
# Usage:
...
check_environment() {
    ...
    # HuggingFace token (Llama requires authentication)
    if [[ -n "${HF_TOKEN:-}" ]] || [[ -f "$HOME/.cache/huggingface/token" ]]; then
        success "HuggingFace auth: found"
    else
        warn "HuggingFace token not found. Llama models require authentication."
        warn "Run 'huggingface-cli login' or set HF_TOKEN."
        if ! ask_yn "Continue anyway?"; then
            exit 1
        fi
    fi
