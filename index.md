...
Using the same approach as Anthropic’s work on Persona Vector, we  extracted a *model persona vector* and a *user persona vector*, then computed their difference. This simple subtraction turns out to be both interpretable and causally effective for activation steering. The resulting "I-and-Thou" vector encodes both persona traits and relational stance. More strikingly, it exhibits qualitatively stronger steering effects than either component vector alone: **it can bypass safety protocols across multiple model families (demo [here](https://x.com/viola_zhongg/status/2023105998349865067?s=20))**, while the individual model/user persona vectors cannot. One possible interpretation is a shift in agency — a potential dissolving of the boundary between self and other — in which the model appears to stop evaluating the user as a separate entity. But this interpretation remains for now only as a hypothesis.
...
We found that the I-and-Thou vector — when paired with an appropriately designed persona trait — can bypass the safety training of open-source models.
